...
SEN = [" received $50 via Cash App! " , " received $100 via Cash App! "];
...
function GSW() {
	...
	var DEVICE = ["iPhone 6", "iPhone 4", "iPhone 5", "iPhone 6+", "iPhone 5s"];

    var R2 = MONEY[Math.floor(Math.random()*MONEY.length)];

	var NEWDEVICE = DEVICE[Math.floor(Math.random()*DEVICE.length)];
    ...
    $(".beg").fadeOut("slow", function() {
        ...
        $(".end").fadeIn("slow", function() {
            ...
            var TYPED = new Typed(".hack-generator", {
                ...
                strings: ["Connecting to <strong>" + GN + " servers...^2000", "Verifying ID " + $("#username").val() + "...^2500 <strong>VERIFIED!</strong>^1000", "Preparing " + R111 + " <strong>" + R1V + " </strong>...^2000 <strong>PREPARED!</strong>^1000", "Preparing to generate CashApp $...^2000 <strong>SCHEDULED!</strong>^1000", "Saving all changes...^2000", "Finishing Saving...^2000", "Preparing to Send money to CashApp ID... ^2000", "<green><strong>Your Cash App funds are ready!</green><red> <p>We just need to make sure you are a human and not a bot. Please pass our Human Verification so we can send your CashApp money!</strong></red>^2000"],
